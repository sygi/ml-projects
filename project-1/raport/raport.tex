...
We started by analyzing the behavior of least squares under $10$-fold cross-validation. Testing cases were:
\begin{itemize}
...
\begin{figure}[!h]
\center
\subfigure[Errors for ridge regression model with all second order terms. Big difference of train and test error tells us that our model has big variance.]{\includegraphics[width=2in]{../figures/ridgeAllPairs-crop.pdf} \label{fig:ridgeAllPairs}}
\hfill
%\subfigure[Train and test error for different values of $\lambda$ in ridge regression. We see that penalizing big $\beta$ values is not justified.]{\includegraphics[width=2.5in]{../figures/ridge-crop.pdf} \label{fig:ridge}}
\caption{}
\end{figure}
