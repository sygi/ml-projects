...
% NOTE: you should do k-fold!
perm = randperm(size(X,1));
split = 0.8;
lastIdx = length(perm) * split;
Tr.idxs = perm(1:lastIdx);
Tr.X = X(Tr.idxs,:);
Tr.y = labels(Tr.idxs);

Te.idxs = perm(lastIdx+1:end);
Te.X = X(Te.idxs,:);
Te.y = labels(Te.idxs);
...
rng(8399);  % fix seed, this    NN is very sensitive to initialization

% setup NN. The first layer needs to have number of features neurons,
%  and the last layer the number of classes (here two).
nn = nnsetup([size(Tr.X,2) 50 2]);
opts.numepochs =  25;   %  Numberdsaof full sweeps through data
